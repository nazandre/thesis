...
public:
  ...
  Time getNextBlockTick(Time now);
	/**
    ...
    // virtual P2PNetworkInterface *getInterfaceForDirection(int direction) { return P2PNetworkInterfaces[i]; }
        /**
	 * @brief Get index of a specific P2PNetworkInterface
	 * @param p2p : pointer to the interface of which the index will be returned
	 * @return index of the P2PNeighborInterface p2p of interface vector
	 */
        int getInterfaceIndex(P2PNetworkInterface* p2p);
	/**
	 ...
	 */
	inline Cell3DPosition getPosition() { return position;};
	...
	unsigned short getNbNeighbors();
  	/**
	 * @brief Prints the neighbor IDs for this block
	 */
