...
string StatsIndividual::getStats() {
  ...
  std::string file("individualStats.dat");
  _export(file);
#endif
  ...
  void StatsIndividual::_export(std::string &outputFile) {
    
    ofstream data;
    map<bID,BuildingBlock*> &modules = getWorld()->getMap();
    map<bID,BuildingBlock*>::iterator it;

    data.open (outputFile, std::ios::out | std::ios::trunc);
    ...
    data.close();
    cerr << "Individual stats exported in " << outputFile << "!" << endl;
  }
}
}
