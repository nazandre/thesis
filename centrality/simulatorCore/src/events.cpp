...
void NetworkInterfaceStopTransmittingEvent::consume() {
	...
	if (!interface->connectedInterface) {
	  ERRPUT << "Warning: connection lost, untransmitted message!" << endl;
	  cerr << "Message "
	       << "(" << interface->messageBeingTransmitted->getMessageName() << ")"
	       << " from " << interface->hostBlock->blockId
	       << " lost!"
	       << endl;
	} else {
	  BaseSimulator::BuildingBlock *receivingBlock = interface->connectedInterface->hostBlock;
	  MessagePtr msg = interface->messageBeingTransmitted;
	  Time now = BaseSimulator::getScheduler()->now();
	  msg->endReceptionMacLayer();
	  
	  receivingBlock->scheduleLocalEvent(EventPtr(new NetworkInterfaceReceiveEvent(now,interface->connectedInterface, msg)));
	...
	interface->availabilityDate = max(BaseSimulator::getScheduler()->now(),
...
//===========================================================================================================
//
//          NetworkDequeueIncomingMsgStatEvent  (class)
//
//===========================================================================================================

NetworkDequeueIncomingMsgStatEvent::NetworkDequeueIncomingMsgStatEvent(Time t,BuildingBlock *conBlock): BlockEvent(t,conBlock) {
  eventType = EVENT_NI_DEQUEUE_INCOMING_MESSAGE_STAT;
  EVENT_CONSTRUCTOR_INFO();
}

NetworkDequeueIncomingMsgStatEvent::~NetworkDequeueIncomingMsgStatEvent() {
  EVENT_DESTRUCTOR_INFO();
}

void NetworkDequeueIncomingMsgStatEvent::consumeBlockEvent() {
  utils::StatsIndividual::decIncommingMessageQueueSize(concernedBlock->stats);
}

const string NetworkDequeueIncomingMsgStatEvent::getEventName() {
  return("NetworkDequeueIncomingMsgStatEvent Event");
}
