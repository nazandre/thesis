...
#ifdef SHOW_MODULES_IN_TARGET
  static int n = 0;
  Cell3DPosition &p = catom2D->position;
  bool inTarget = BlockCode::target->isInTarget(p);
  if (inTarget) {
    Color c = BlockCode::target->getTargetColor(p);
    catom2D->setColor(c);
    n++;
  } else {
    Color c(0,0,0,0);
    catom2D->setColor(BLACK);
  }
  cout << "n: " << n << endl;
#endif
