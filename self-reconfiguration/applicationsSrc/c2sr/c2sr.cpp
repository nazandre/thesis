...
#ifdef MAP_REQUEST_STATS
  stats = new MapRequestStats();
#endif
}
...
bool C2SR::isInStream() {
  ...
  if (!isInTarget(src) && (pivot.position.y == 0)) { // ground
    return true;
  } else if (!isInTarget(src) && (dest.y <= src.y)) { // descent over I
    return true;
  }

  /*
  } else if (!isInTarget(src) && isInTarget(pivot.position)) { // 
    return true;
  } else if (isInTarget(src) && isInTarget(dest) && (dest.y <= src.y)) {
    return true;
    } */
  else {
    return false;
...
bool C2SR::checkConvergence() {
  ...
  if ((isInTarget(src) && !isInTarget(dest)) ||
      (isInTarget(src) && isInTarget(dest) && (dest.y > src.y))) {
    return true;
  }
  return false;
}
...
void C2SR::start() {
  ...
  if (!started) {
    ...
    started = true; 
    if (isInTarget(map->position)) { // seed
      hasConverged();
    } else if (isInStream()) {
      setState(WAITING);
      requestClearance();
...
bool C2SR::isInTarget(Coordinate p) {
#ifdef MAP_REQUEST_STATS
  stats->insertRequest(p);
#endif
  return Map::isInTarget(p);
}
